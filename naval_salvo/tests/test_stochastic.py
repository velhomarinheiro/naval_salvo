"""Testes do módulo naval_salvo.stochastic.

Cobertura:
    - Reprodução exata do exemplo determinístico de Armstrong (2014, p.1594)
    - Aderência ao cenário estocástico publicado (A=6, p=0.67, p.1597)
    - Propriedade de ordenação: 1º >= simultâneo >= 2º (em média)
    - Imunidade ciber de submarinos (χ)
    - Truncamento de perdas em [0, estoque]
    - Reprodutibilidade por semente
    - Veredito sobre forças navais apenas
"""
...
def md_forces(cyber_blue=0.0, cyber_red=0.0):
    blue = MultiDomainForce("Azul", [
        UnitGroup("fragata", "s", 4, 4, 0.67, 2, 0.67, 0.33, 0.11),
        UnitGroup("aeronave", "a", 2, 2, 0.75, 0, 0.0, 0.5, 0.15),
    ], cyber_offense=cyber_blue)
    red = MultiDomainForce("Vermelho", [
        UnitGroup("escolta", "s", 5, 4, 0.60, 2, 0.60, 0.33, 0.11),
        UnitGroup("submarino", "u", 1, 4, 0.80, 0, 0.0, 0.5, 0.15),
    ], cyber_offense=cyber_red)
    return blue, red


# ---------------------------------------------------------------------------
# Validação Armstrong (2014) — caso determinístico
# ---------------------------------------------------------------------------
class TestDeterministicArmstrong2014:
    ...
    def test_simultaneous_matches_published(self):
        ...
        assert np.allclose(bl, 4.0, atol=1e-9)   # Azul aniquilado
        assert np.allclose(rl, 2.0, atol=1e-9)
...
# ---------------------------------------------------------------------------
# Validação Armstrong (2014) — cenário estocástico publicado
# ---------------------------------------------------------------------------
class TestStochasticArmstrong2014:
    """Simulação publicada: média 2.637, dp 0.989, P[0]=0.8%, P[all]=12.5%."""
...
# ---------------------------------------------------------------------------
# Modulação ciber e imunidade submarina
# ---------------------------------------------------------------------------
class TestCyberModulation:
    ...
    def test_submarine_immune_to_cyber(self):
        """Sob ciber Vermelho forte, apenas o submarino mantém eficácia."""
    ...
    def test_cyber_dominance_can_invert_outcome(self):
        """Consistente com o achado de sensibilidade do modelo determinístico."""
...
class TestInvariants:
    ...
    def test_naval_verdict_ignores_air_units(self):
        """Aeronaves não contam para o veredito (força naval = s + u)."""
        blue, _ = md_forces()
        stocks = blue.initial_stocks()
        assert blue.naval_strength(stocks) == pytest.approx(4.0)  # só fragatas
